...
export default function NewsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await getPaginatedPosts('news', 1, 9999);
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.hero}>
        <div className={styles['hero__container']}>
          <div className={styles['hero__inner']}>
            <div className={styles['hero__icon']}><GlobalOutlined /></div>
            <div className={styles['hero__text']}>
              <h1>Tin tức{activeCategory ? ` — ${activeCategory}` : ''}</h1>
              <p>Sự kiện, drama được tổng hợp từ các báo chính thống</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={styles.toolbar}>
        ...
        {!loading && (
          <span className={styles['count-badge']}>{filtered.length} bài</span>
        )}
      </div>
          ...
          <>
            <div className={styles.grid}>
              {paginated.map((post, idx) => (
                <PostCard key={post.id ?? idx} post={post} variant="news" />
              ))}
            </div>
